...
bson_file_path = '/media/dereyly/data/data/train(1).bson'
...
def process(q, iolock):
    while True:
        ...
        for e, pic in enumerate(d['imgs']):
            im_path_old=os.path.join(image_path, str(product_id) + "_" + str(e) + ".png")
            im_path=os.path.join(image_path,str(category_id), str(product_id) + "_" + str(e) + ".png")
            if not os.path.exists(im_path) and  not os.path.exists(im_path_old):
                print('cant move file',im_path_old)
                continue
            if not os.path.exists(im_path):
                os.rename(im_path_old,im_path)
            zz=0
            #picture = imread(io.BytesIO(pic['picture']))
            #imsave(im_path, picture)
