...
def read_batch_data(data_batch):
    img_batch = []
    targets = []
    indexes = []
    count = 0
    for c, d in enumerate(data):
        ...
        for e, pic in enumerate(d['imgs']):
            ...
            # #
            image_path='/home/dereyly/ImageDB/cdiscount/train/'
            path=os.path.join(image_path, str(category_id),str(product_id) + "_" + str(e) + ".png")
            # # with open(path, 'rb') as f:
            # #     with PIL.Image.open(f) as pil_img:
            # #         pil_img.convert('RGB')
            pil_img=PIL.Image.open(path)
            pil_img.convert('RGB')
            #img2=np.asarray(pil_img, np.float32)
            img2=np.array(pil_img)
            # #img=img.astype(np.float32)
            # diff=img-img2
            # cv2.imwrite('/home/dereyly/ImageDB/1_%d_%d.png' %(product_id,e), img)
            # cv2.imwrite('/home/dereyly/ImageDB/2_%d_%d.png'%(product_id,e), img2)
            # print(diff.sum())
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        for i in range(len(img_batch)):
            ...
            if k>=batch_size or i==len(img_batch)-1:
                ...
                for j in range(k):
                    ...
                    zz=0
                k=0
                # dtarget=data_dbg['target']-tmp_target_batch
                # dout=out-data_dbg['out']
                # dinput=data_dbg['data']-input_batch.numpy()
                # ddout=dout.sum(axis=1)
                # print(dtarget.sum(),dout.sum(),dinput.sum())
                input_batch = torch.FloatTensor(batch_size, 3, 160, 160)
            zz=0
        if is_val:
        ...
        #print(indexes)
    pkl.dump(data_tst,open('/home/dereyly/ImageDB/cdiscount/val_small.pkl','wb'))
