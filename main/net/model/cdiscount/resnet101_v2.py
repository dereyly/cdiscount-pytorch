...
class ClassifyBlock_v5(nn.Module):
    def __init__(self, inplanes, num_classes=[1000]):
        ...
        self.fc_m2 = nn.Linear(4096, num_classes[3])
        self.fc_m3 = nn.Linear(4096, num_classes[4])
        self.fc_m4 = nn.Linear(4096, num_classes[5])
        self.fc_m5 = nn.Linear(4096, num_classes[6])
        self.fc_m6 = nn.Linear(4096, num_classes[7])
        self.fc_m7 = nn.Linear(4096, num_classes[8])
        self.fc_m8 = nn.Linear(4096, num_classes[9])
...
class ClassifyBlock(nn.Module): #dereyly code
...
class ResNet101(nn.Module):
    ...
    def __init__(self, in_shape=(3,180,180), num_classes=[5270], extend = False, experemental=False):
        ...
        if experemental:
            ...
            self.ex_classify=ClassifyBlock(2048,num_classes=num_classes[0])
        else:
            if not extend:
                self.fc  = nn.Linear(2048, num_classes[0])
            else:
                self.fc = nn.Linear(2048, num_classes[0])
                self.ex_classify=ClassifyBlock_v5(2048,num_classes=num_classes)
